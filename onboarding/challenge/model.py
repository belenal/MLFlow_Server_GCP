...
from onboarding.challenge.feature_selector import TopFeatureSelector
...
class DelayModel:
    ...
    def preprocess(self, features: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.selector is None:
            matched_features = [
                name for name in feature_names if any(top in name for top in self.top_10_features)
            ]
    ...
    def save_model(self, path: str) -> None:
        if self._model is None:
            ...
            raise ValueError(error_msg)
        joblib.dump(
            {"model": self._model, "preprocessor": self.preprocessor, "selector": self.selector},
            path,
        )
